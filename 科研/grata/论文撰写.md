...
我们遵循 ProSFDA \cite{hu2022minmax} 的数据预处理与增广流程，在五个眼底数据集（RIM-ONE-r3、REFUGE、ORIGA、REFUGE-Validation/Test、Drishti-GS）上进行单源到多目标的跨域评估：对每个源域单独训练 ResUNet-34 基线模型，并在其余 4 个目标域上进行测试时自适应（TTA），最终报告 5×4=20 组结果的平均值。为公平比较，按 Yang 等（2022）\cite{yang2022dltta} 对每个测试批次采用 batch size=1 的单迭代适应；部署 MASR 时，仅优化 BN 仿射参数并使用 Adam（lr=1e-4）更新，同时按 Wang 等（2021）\cite{wang2021tent} 在目标域上重新估计 BN 统计量。主伪损失由一致性项与语义原型对比项加权组成，权重 β=1.0。我们与以下方法对比：No Adapt、DUA \cite{mirza2022dua}、DIGA \cite{wang2023diga}、MedBN \cite{park2024medbn}、SAR \cite{niu2023stabletta}、DeTTA \cite{wen2023denoisingtta} 与 GraTa \cite{chen2024grata}。所有方法均在相同的输入尺寸（512×512）与评价流程下复现。
