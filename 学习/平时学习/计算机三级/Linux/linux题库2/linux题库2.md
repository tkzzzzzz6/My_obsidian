...
**答疑：【解析】**在 Unix/Linux 系统中，`PS1` 是一个环境变量，用于定义命令行提示符的格式。命令 `PS1="[\u\w]\$"` 将 `PS1` 设置为 `[\u\w]\$`，其中：
...
**答疑：【解析】**在 Linux 系统中，`chmod` 命令用于改变文件或目录的权限。权限用数字表示时：
...
| **cpio / tar**   | `cpio -o < filelist            | cpio -i`                                                          | 归档/复制一批文件，常和 `find` 配合使用 | 支持批量操作                   | 用法相对复杂               |
...
✅ **补充说明：**在现代 Linux 中，如果是 SATA 或 SCSI 硬盘，命名会是 `/dev/sda`、`/dev/sdb`，其规则和 IDE 类似。
...
45. 用一个命令行读取文件 file 第 5 ~ 10 行 **`head -10 file | tail -6`**
    **解析：**使用 `head -10 file` 提取文件的前 10 行，然后通过 `tail -6` 管道将前 10 行输出给 `tail -6`，截取其中的后 6 行（即原文件的第 5~10 行）。因此正确命令是：`head -10 file | tail -6`
