...
4. PowerDrill 与 Dremel 的差异
   - PowerDrill 尽可能将数据加载到内存，计算方式更接近内存计算。

5. PowerDrill 的数据结构
   - 使用双层数据字典结构。
   ![1768208566791.png](https://tk-pichost-1325224430.cos.ap-chengdu.myqcloud.com/blog/1768208566791.png)

6. 冷热数据分离策略
   - 冷数据：访问频率低的数据。
   - 热数据：访问频率高的数据。

7. 游程编码（Run-Length Encoding）
   - 将连续重复的数据用“值 + 连续次数”表示，例如 `AAAAABBB` 可编码为 `A5B3`。
   - 适合重复度高的数据列，减少存储与内存占用，提高扫描效率。

8. 汉明距离
   - 在信息论中，两个等长字符串之间的汉明距离是对应位置不同字符的个数。
   ![1768209049597.png](https://tk-pichost-1325224430.cos.ap-chengdu.myqcloud.com/blog/1768209049597.png)

9. Dremel 与 PowerDrill 对比
   ![1768208960087.png](https://tk-pichost-1325224430.cos.ap-chengdu.myqcloud.com/blog/1768208960087.png)
# GAE
...
2. Dynamo 需要解决的主要问题及解决方案
   ![1768217364589.png](https://tk-pichost-1325224430.cos.ap-chengdu.myqcloud.com/blog/1768217364589.png)

3. 一致性哈希算法
   - 数据存放到顺时针方向的第一个节点。
   - 优点：新增/删除节点仅影响其前驱节点，其他节点不受影响，数据迁移开销小。
   ![1768217444009.png](https://tk-pichost-1325224430.cos.ap-chengdu.myqcloud.com/blog/1768217444009.png)
   - 引入虚拟节点与数据分区后：
     - 降低数据分布不均衡的概率。
     - 节点变更时数据迁移进一步减少。
   ![1768217580736.png](https://tk-pichost-1325224430.cos.ap-chengdu.myqcloud.com/blog/1768217580736.png)
# AWS-EC2-S3

1. EC2（Elastic Compute Cloud）概述
   - AWS 重要组成部分，用于提供弹性可调的计算容量。
   - 具备低成本、灵活性、安全性、易用性与容错性。
   - 用户无需硬件投入即可快速开发、部署与管理应用。

2. EC2 基本架构
   ![1768218185299.png](https://tk-pichost-1325224430.cos.ap-chengdu.myqcloud.com/blog/1768218185299.png)
...
5. EC2 的关键技术
   ...
   - EC2 通信机制：
     ![1768218463752.png](https://tk-pichost-1325224430.cos.ap-chengdu.myqcloud.com/blog/1768218463752.png)
...
7. S3（Simple Storage Service）概述
...
2. SimpleDB 与 DynamoDB 的比较
   ...
   - DynamoDB：
     ...
     - 不限制单表数据量，适合大规模负载。
# AWS-RDS
1. Amazon RDS将MySQL数据库移植到集群中，在一定的范围内解决了关系数据库的可扩展性问题。
2. MySQL集群方式采用了Share-Nothing架构。每台数据库服务器都是完全独立的计算机系统，通过网络相连，不共享任何资源。

3. SQS的消息格式
> (1)消息ID:由系统返回给用户，用来标识队列中的不同消息。
> (2)接收句柄:当从队列中接收消息时就会从消息那里得到一个接收句柄，这个句柄可以用来对消息进行删除等操作。
> (3)消息体:消息的正文部分，需要注意的是消息存放的是文本数据并且不能是URL编码方式。
> (4)消息体MD5摘要:消息体字符串的MD5校验和。

4. CDN:通过将网站内容发布到靠近用户的边缘节点，使不同地域的用户在访问相同网页时可以就近获取。
# AWS-其他
