...
> - 不间断的监控:Google的服务是全天候的，如果不能对Google 的后台同样进行全天候的监控，很可能会错过某些无法再现的关键性故障

2. Dapper的三个基本设计目标
> (1)低开销:这个是广泛可部署性的必然要求。监控系统的开销越低，对于原系统的影响就越小，系统的开发人员也就越愿意接受这个监控系统。
> (2)对应用层透明:监控系统对程序员应当是不可见的。如果监控系统的使用需要程序开发人员对其底层的一些细节进行调整才能正常工作的话，这个监控系统不是一个完善的监控系统。
> (3)可扩展性:Google的服务增长速度是惊人的，设计出的系统至少在未来几年里要能够满足Google服务和集群的需求。

3. 监控系统通常有两种方案：
> (1)黑盒（Black Box）:比较轻便，通过统计学来判断消息关系，不够准确。
> (2)基于注释的监控（Annotation-based Monitoring）:利用应用程序或中间件给每条记录赋予一个全局性的标示符，借此将相关消息串联起来。
>
> Google选择了基于注释的方案。为了尽可能消除监控系统的应用程序对被监控系统的性能产生的不良影响，Google设计并实现了一套轻量级的核心功能库。

4. Dapper的三个基本概念:
> (1)监控树（Trace Tree）:一个同特定事件相关的所有消息，以树的形式组织起来。
> (2)区间（Span）:树中的每个节点就是一个区间，区间实际上就是一条记录，所有记录联系起来就是对某个事件的完整记录。
> ![1768207513735.png](https://tk-pichost-1325224430.cos.ap-chengdu.myqcloud.com/blog/1768207513735.png)
>
> 监控id对每棵树的所有区间是相同的，id内容随机分配，且在整个Dapper监控系统中是唯一的，用于区分不同的监控。
> (3)注释（Annotation）:主要用来辅助推断区间关系，也可以包含一些自定义的内容。

1. Dapper的三个设计目标和两个关键技术:
>- 三个设计目标:低开销,对应用层透明(难度最大),可拓展性
>- 两个关键技术:轻量级的核心功能库,二次抽样技术

6. Dapper的使用场景
>![1768207842738.png](https://tk-pichost-1325224430.cos.ap-chengdu.myqcloud.com/blog/1768207842738.png)
# Dremmel
1. Dremel的产生背景:
...
1. Dremel是第一个在嵌套数据模型基础上实现列存储的系统。
> 列存储的主要好处在于：
> - 处理时，只需要使用涉及的列数据。
> - 更利于数据的压缩。
> 
